...
-- dau vao p_patient_id ma benh nhan,p_product_id ma san pham, p_quantity so luong mua
-- dau ra OUT p_message chuoi tbao trang thai
-- sdung out vi procedure can tra kqua ra ngoai cho he thong hien thi

DROP PROCEDURE IF EXISTS ProcessEquipmentPurchase;
DELIMITER //
CREATE PROCEDURE ProcessEquipmentPurchase(IN p_patient_id INT,IN p_product_id INT,IN p_quantity INT,OUT p_message VARCHAR(100))
BEGIN
    DECLARE v_stock INT;
    ...
    IF v_stock < p_quantity THEN
        SET p_message = 'Kho khong du spham';
        ROLLBACK;
    ELSEIF v_status = 'Inactive' THEN
        SET p_message = 'Vi dang bi khoa';
        ROLLBACK;
    ELSEIF v_balance < v_total THEN
        SET p_message = 'So du khong du';
        ROLLBACK;
    ELSE
        START TRANSACTION;
            UPDATE Products
            ...
            WHERE patient_id = p_patient_id;
        COMMIT;
        SET p_message = 'Da xu li don hang';
    END IF;
END //
DELIMITER ;
...
SELECT @msg;
CALL ProcessEquipmentPurchase(1, 2, 10, @msg);
SELECT @msg;
